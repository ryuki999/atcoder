...
class AbstractDrawer(object):
    def __init__(self, XY, rec_type, top_n, loop):
    ...
    def draw(self, qs):
        ...
        while True:
            ...
            while True:
                ...
                # 傾き正方向
                x_xy = self.search_cor(start_cor, direction=self.x_dir, xy=self.XY[x_iter])
                ...
                while True:
                    ...
                    # 傾き負方向
                    y_xy = self.search_cor(start_cor, direction=self.y_dir, xy=self.XY[y_iter])
    ...
    def _append_qs(self, qs_tmp, q, cs):
        ...
        for _, q in cs.items():
            ...
            if self._contain_target_cor(q[0], q[1]) and self._contain_target_cor(q[0], q[3]) and q[0] not in self.XY:
                if sum(j == {self.rec_type: q} for j in qs_tmp) < 1 and self._check_cor_line(qs_tmp, q):
                    if self.loop < self.q1_threshhold and self._promise_q1(qs_tmp, q[0]):
                        continue
                    self.XY.append(q[0])
    ...
    def _promise_q1(self, qs_tmp, q1):
        x_iter = -1
        while True:
            x_iter += 1
            if x_iter >= len(self.XY):
            ...
            # 傾き正方向
            x_xy = self.search_cor(q1, direction=self.x_dir, xy=self.XY[x_iter])
            if self.XY[x_iter] == q1 or x_xy is None:
                continue
            ...
            while True:
                ...
                # 傾き負方向
                y_xy = self.search_cor(q1, direction=self.y_dir, xy=self.XY[y_iter])
                if self.XY[y_iter] in q or y_xy is None:
                    continue
                q = q + [y_xy]
                q1 = (q[1][0] - (q[1][0] - q[0][0]) - (q[1][0] - q[2][0]), q[1][1] - (q[1][1] - q[0][1]) - (q[1][1] - q[2][1]))
...
class TransverseDirectionDrawer(AbstractDrawer):
    def __init__(self, XY, top_n):
        super().__init__(XY=XY, rec_type="tdd", top_n=top_n, loop=loop)
        self.x_dir = 1
        self.y_dir = 0
        self.q1_threshhold = 2
...
class ObliqueDirectionDrawer(AbstractDrawer):
    def __init__(self, XY, top_n, loop):
        super().__init__(XY=XY, rec_type="odd", top_n=top_n, loop=loop)
        self.x_dir = 1
        self.y_dir = -1
        self.q1_threshhold = 2
...
# for _ in range(NUM):
    ...
    odd = ObliqueDirectionDrawer(XY, top_n=top_n, loop=loop)
    XY, qso  = odd.draw(qso)
    for q in qso:
        if q not in qs:
            qs += [q]
    ...
    if runTime + (endTime - intervel_time) > 4.8:
        break
    # print(loop)
