...
def check_target_cor(cor1, cor2, target_cor):
    for i in range(cor1[0], cor2[0]):
...
def check_cor_line(qs, q):
    ...
    for qi in q:
        for qsi in qs:
            if not check_target_cor(qsi[0], qsi[1], qi):
                return False
            if not check_target_cor(qsi[1], qsi[2], qi):
                return False
            if not check_target_cor(qsi[2], qsi[3], qi):
                return False
            if not check_target_cor(qsi[3], qsi[0], qi):
                return False
    return True
